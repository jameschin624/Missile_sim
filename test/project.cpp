...
float cAcc=0.225; //coefficient for the degree of acceleration function
...
int OFFSET=nCollect-30; //number of missile data ignored from beginning
...
class statistics
{
	...
	statistics(int t,int m,int n) //n:number of data  m:which missile track file to be analyzed
	{
		FILE *fp1,*fp2;
		int i;
		char fn[30];
		...
		data1=new point[t];
		data2=new point[t];
		data=new point[t-nCollect];
		
		for (i=0;i<t;i++)
		{
			fscanf(fp1,"%f %f %f",&(data1[i].x),&(data1[i].y),&(data1[i].z));
		    cout<<data1[i].x<<" "<<data1[i].y<<" "<<data1[i].z<<endl;
			
		}
		sprintf(fn,"track%d.txt",n);
		fp2=fopen(fn,"r");
		if (fp2==NULL) return;
		cout<<"************"<<endl;
		for(i=0;i<t;i++)
		{
			fscanf(fp2,"%f %f %f",&(data2[i].x),&(data2[i].y),&(data2[i].z));
			cout<<data2[i].x<<" "<<data2[i].y<<" "<<data2[i].z<<endl;
		}
		fclose(fp1);
		fclose(fp2);
		cout<<"************"<<endl;
		for(i=0;i<t-nCollect;i++)
		{
			data[i].x=abs(data1[i+nCollect].x-data2[(i+nCollect)-1].x);
			data[i].y=abs(data1[i+nCollect].y-data2[(i+nCollect)-1].y);
			data[i].z=abs(data1[i+nCollect].z-data2[(i+nCollect)-1].z);
...
int main()
{
	...
	//file=fopen("standard.txt","w");
	file1=fopen("number of collected data.txt","w");
	//if (file==NULL) return;
	nCollect=50;
	for(int i=1;i<5;i++)
	{
...
void test1()
{
	...
	void fly()
	{
		int i;
		vector v1,v2; 
		missile m1(1);//m3('r'); //create a missile with parameters in file missile1.txt and missile2.txt, respectively
		interceptor m2(1);//create a interceptor with parameters in file interceptor1.txt
		// interceptor m2 模擬 missile m1 的軌跡, 綠色為收集資料階段的軌跡, 粉紅色則為預估軌跡 
		FILE *fp1,*fp2,*fp3;
		char tt[30];
	    //sprintf(filename,"track3%d.txt",startplus);
		fp1=fopen("track1.txt","w");
		if (fp1==NULL) return;
		fp2=fopen("track2.txt","w");
		if (fp2==NULL) return;
		fp3=fopen("track3.txt","w");
		if (fp3==NULL) return;
		while(v1.target.z>=0 || v2.target.z>=0) //missile 在地面上時顯示, 落地後則停止 
		{
			if (v1.target.z<0) m1.on=0; 
		//	m1.show(15);
			fprintf(fp1,"%f %f %f\n",m1.current.x,m1.current.y,m1.current.z); //收集 missile m1 之資料 (前 nCollect 筆) 
			fprintf(fp2,"%f %f %f\n",m1.current.x,m1.current.y,m1.current.z);
			if (tCount==nCollect) 
			{
				fclose(fp1); //資料收集完畢 
				m2.fire(1); //interceptor m2 對 missile m1 的 track1.txt  作分析 
				fprintf(file1,"%f %f %f\n",m2.xabc[0].x,m2.xabc[0].y,m2.xabc[0].z);
			}
		...
		//	m2.show(0);
			...
			{
				v2=m2.pNext(tDelta);
				//cout<<v2.target.z<<endl;
				//fprintf(fp3,"%f %f %f\n",v2.target.x,v2.target.y,v2.target.z);
				//fclose(fp3);
			}
			fprintf(fp3,"%f %f %f\n",v2.target.x,v2.target.y,v2.target.z);
			tCount++;		       
	...
	void standard(int tCount,int m,int n) 
	{
		point a;
		statistics std1(tCount,m,n);
	    a.x=std1.sd().x;
	    a.y=std1.sd().y;
	    a.z=std1.sd().z;
	    //cout<<std1.sd().x<<"       "<<std1.sd().y<<"    "<<std1.sd().z<<endl;
		//a[i]=std1.sd().x;
		fprintf(file,"%f %f %f\n",a.x,a.y,a.z);
		return ;
	}
